...
export const MATCH_ANALYSIS_SYSTEM_PROMPT = ANALYZE_SYSTEM_PROMPT;

export const DEEP_ANALYSIS_RESPONSE_SCHEMA_TEXT = `{
...
export const MATCH_ANALYSIS_RESPONSE_SCHEMA_TEXT = ANALYZE_RESPONSE_SCHEMA;
...
export type MatchAnalysisUserPromptInput = {
  ...
  /** Selects lean Analyze vs existing Deeper Analysis prompt/schema. Default: analyze. */
...
function deepClosingInstructions(resumeChars: number): string {
  const sizeLimit =
    resumeChars < 8_000
      ? ""
      : `
...
function closingInstructions(input: MatchAnalysisUserPromptInput): string {
  if (resolveAnalysisMode(input) === "deep") {
    return deepClosingInstructions(input.resumeText.length);
  }
  return analyzeClosingInstructions(input.resumeText.length);
}
...
export function truncateStrengthsAndGaps(
  ...
  const questionLimit = analysisMode === "analyze" ? 4 : 5;
  if (resumeChars <= 8000 && analysisMode === "analyze") {
    return {
      ...analysis,
