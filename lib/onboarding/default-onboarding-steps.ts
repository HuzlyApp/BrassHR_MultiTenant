import type { OnboardingStepType } from "@/lib/onboarding/types";

export type OnboardingStepDraft = {
  step_key: string;
  title: string;
  description: string;
  step_type: OnboardingStepType;
  sort_order: number;
  is_required: boolean;
  is_enabled: boolean;
  metadata: Record<string, unknown>;
  required_documents: Array<{
    title: string;
    description: string;
    is_required: boolean;
    sort_order: number;
  }>;
};

export function createDefaultOnboardingStepDrafts(): OnboardingStepDraft[] {
  return [
    {
      step_key: "resume_upload",
      title: "Upload Resume",
      description: "Upload your resume and confirm your contact information.",
      step_type: "resume_upload",
      sort_order: 10,
      is_required: true,
      is_enabled: true,
      metadata: {
        parsing_enabled: true,
        required: true,
        workflow_step_id: "resume-basic-profile",
      },
      required_documents: [],
    },
    {
      step_key: "professional_license",
      title: "Professional License",
      description: "Upload professional license documents",
      step_type: "professional_license",
      sort_order: 20,
      is_required: true,
      is_enabled: true,
      metadata: { workflow_step_id: "credential-license-verification" },
      required_documents: [
        {
          title: "Nursing License",
          description: "Front and back if applicable",
          is_required: true,
          sort_order: 10,
        },
        {
          title: "TB Test",
          description: "Within the last 12 months",
          is_required: true,
          sort_order: 20,
        },
        {
          title: "CPR Certifications",
          description: "",
          is_required: true,
          sort_order: 30,
        },
      ],
    },
    {
      step_key: "skill_assessment",
      title: "Skill Assessment",
      description: "Complete the skills assessment",
      step_type: "skill_assessment",
      sort_order: 30,
      is_required: true,
      is_enabled: true,
      metadata: { workflow_step_id: "skill-qualification-assessment" },
      required_documents: [],
    },
    {
      step_key: "authorization_background_check",
      title: "Authorization / Background Check",
      description: "Complete authorization and background screening requirements",
      step_type: "custom_question",
      sort_order: 40,
      is_required: true,
      is_enabled: true,
      metadata: { workflow_step_id: "background-check" },
      required_documents: [],
    },
    {
      step_key: "agreement_signature",
      title: "Agreement / Signature",
      description: "Review and sign required agreements",
      step_type: "authorizations",
      sort_order: 50,
      is_required: true,
      is_enabled: true,
      metadata: { workflow_step_id: "employee-agreement" },
      required_documents: [],
    },
    {
      step_key: "review_submit",
      title: "Final Review / Completion",
      description: "Review your application and complete onboarding",
      step_type: "review_submit",
      sort_order: 60,
      is_required: true,
      is_enabled: true,
      metadata: { workflow_step_id: "completion-milestone" },
      required_documents: [],
    },
  ];
}

export function reindexStepSortOrders(steps: OnboardingStepDraft[]): OnboardingStepDraft[] {
  return steps.map((s, i) => ({ ...s, sort_order: (i + 1) * 10 }));
}
