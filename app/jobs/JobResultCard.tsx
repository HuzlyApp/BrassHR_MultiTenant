  ...
  return (
    <article>
      <button
        ...
        onClick={onSelect}
        className={`relative w-full rounded-lg border px-3.5 py-3 text-left transition motion-reduce:transition-none min-h-11 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[color:var(--brand-primary)] focus-visible:ring-offset-2 ${
          selected
            ? "border-[color:color-mix(in_srgb,var(--brand-primary)_28%,#e2e8f0)] bg-[color:color-mix(in_srgb,var(--brand-primary)_6%,white)]"
      ...
      >
        {selected ? (
          <span
            aria-hidden
            className="absolute inset-y-0 left-0 w-1 rounded-l-lg bg-[color:var(--brand-primary)]"
          />
        ) : null}
        <div className="flex items-start justify-between gap-3 pl-2">
          <div className="min-w-0">
            <p
              className={`line-clamp-2 ${JOB_POSTING_CARD_TITLE_CLASS} ${
                selected ? "text-[color:var(--brand-primary)]" : ""
              }`}
            >
              {title}
            </p>
          ...
          {job.employment_type ? (
            <span className={`shrink-0 rounded-md bg-slate-100 px-2 py-0.5 text-slate-600 ${JOB_POSTING_BADGE_CLASS}`}>
              {job.employment_type}
            </span>
          ) : null}
        </div>
