-- Re-seed global skill assessment catalog (categories + questions).
-- Idempotent upserts; safe when the original seed migration was never applied.
...
-- Ensure anon/authenticated clients can read the shared catalog.
