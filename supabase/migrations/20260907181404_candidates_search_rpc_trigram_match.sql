-- Candidates search RPC: trigram-friendly column matches, LIKE escaping, multi-token names.
-- Semantics unchanged: p_search ORs fields; p_skills ANDs skills; both set → AND.
          ...
          SELECT bool_and(
            EXISTS (
              ...
              WHERE wps2.worker_id = w.id
                ...
                AND lower(wps2.skill_name) LIKE '%' || replace(replace(skill, '%', '\%'), '_', '\_') || '%' ESCAPE '\'
            )
            OR EXISTS (
              SELECT 1
              ...
              WHERE wr2.worker_id = w.id
                ...
                AND (
                  lower(coalesce(wr2.extracted_text, '')) LIKE '%' || replace(replace(skill, '%', '\%'), '_', '\_') || '%' ESCAPE '\'
                  OR (
                    length(skill) >= 3
                    AND to_tsvector('english', coalesce(wr2.extracted_text, ''))
...
COMMENT ON FUNCTION public.list_candidate_ids_page IS
  'Tenant-scoped paginated candidate IDs. Free-text (p_search) ORs across name/email/phone/role/apps/resume/skills using trigram-friendly column matches. Skills (p_skills) AND together; when both set, free-text AND skills. Never cross-tenant. SECURITY INVOKER.';
...
-- PostgREST must refresh its schema cache after CREATE OR REPLACE FUNCTION.
