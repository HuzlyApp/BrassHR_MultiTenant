-- Remote jobs: empty remote_allowed_states means All States (nationwide).
-- Recruiters only populate the array when restricting to specific states.
-- Platform holds still apply when a restricted list includes CA / IL / CT.
...
BEGIN
  ...
  IF v_type IN ('remote') THEN
    -- Empty = All States. Only gate when an explicit restricted list includes held states.
    v_remote := COALESCE(NEW.remote_allowed_states, ARRAY[]::text[]);
    IF array_length(v_remote, 1) IS NOT NULL AND EXISTS (
      SELECT 1
...
COMMENT ON FUNCTION public.enforce_open_job_service_area() IS
  'Phase 1 backstop: block open/published jobs in CA, IL, CT, or NYC (including hybrid remote states). Empty remote_allowed_states means All States. Drafts are allowed.';
