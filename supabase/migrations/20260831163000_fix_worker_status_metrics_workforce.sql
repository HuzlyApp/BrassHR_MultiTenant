-- Workforce analytics: count employment records from public.workers (not recruitment candidates in public.worker).
-- Previously new/pending candidates were incorrectly bucketed as on_leave.
...
AS $$
  ...
  SELECT jsonb_build_object(
    ...
    'offer_extended', (
      SELECT count(*) FROM pipeline WHERE display_status = 'approved'
    ),
    'hires', (
      SELECT count(*) FROM employment
      WHERE display_status IN ('active', 'new')
    ),
    'pending_workers', (
